...
xhr.onload = () => {
  if (xhr.status === 200) {
    ...
    for (const service of serviceHistory) {
      ...
      html += `<td><button class="edit-service-button" data-service-id="${service.id}">Edit</button>`;
      html += `<button class="delete-service-button" data-service-id="${service.id}">Delete</button></td>`;
      html += '</tr>';
    }
    html += '</tbody>';
    html += '</table>';
